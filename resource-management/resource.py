...
def eg_rule_action_await(memories_info: List["MemoryInfo"], args):
    ...
    memory = memories[0]
    protocol = EntanglementGenerationA.create(None, "EGA." + memory.name, mid_name,
                                               other_name,
                                               memory)
...
for i, info in enumerate(r1.resource_manager.memory_manager):
    print("{:6}\t{:15}\t{:9}\t{}".format(str(i), str(info.remote_node), str(info.fidelity), str(info.entangle_time * 1e-12)))
